...
all_chords = list(sorted(set(major_stat_set.all_chords()) | set(parallel_minor_stat_set.all_chords()) | set(
    relative_minor_stat_set.all_chords())))
    ...
    async for message in websocket:
        ...
        try:
            ...
            if not constraints:
                ...
                constraints = [{'time': i * chord_length, 'locked': False} for i in
                               range(1 + int(last_end // chord_length))]
                preserve = False
            ...
            locked_chords = [
                Chord.parse(constraint['value']).absolute_to_relative(key_signature) if constraint['locked'] else None
                for constraint in constraints]

            if preserve:
                preserve_chords = [Chord.parse(constraint['value']).absolute_to_relative(key_signature) for constraint
                                   in constraints]
            else:
            ...
            if mode == 'major':
                ...
                stat_set_list = [(1.0 - minorness, major_stat_set), (minorness, parallel_minor_stat_set)]
            elif mode == 'mixed-relative':
                stat_set_list = [(1.0 - minorness, major_stat_set), (minorness, relative_minor_stat_set)]
            else:
                stat_set_list = [(1.0, major_stat_set)]  # ?????

            chords = linearly_mixed_hmm_predict(stat_set_list, grouped_notes, locked_chords, preserve_chords,
                                                jazziness=jazziness, seed=seed, first_note_weight=first_weight,
            ...
            for i, ((chord_score, chord), suggestion, scored_chord_list) in enumerate(chords):
                res.append({
                    ...
                    'suggestion': productionize_chord(suggestion[1], key_signature, suggestion[0],
                                                      bottom_bass) if suggestion else None,
                    'locked': i < len(locked_chords) and locked_chords[i] is not None,
                    'recommendations': [productionize_chord(c, key_signature, s, bottom_bass) for (s, c) in
                                        scored_chord_list],
                })
            print('------------------------')
            print('result:', res)
